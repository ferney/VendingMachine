...
public class VendingMachine
{
    ...
    private bool CheckChangeAvailability(int changeAmount)
    {
        ...
        // Verifica si hay suficientes monedas de 500 para el cambio
        while (remainingChange >= 500 && coins500.Count() > 0)
        {
            remainingChange -= 500;
            coins500.Pop(); // Elimina una moneda de 500 del stock
        }

        // Verifica si hay suficientes monedas de 200 para el cambio
        while (remainingChange >= 200 && coins200.Count() > 0)
        {
            remainingChange -= 200;
            coins200.Pop(); // Elimina una moneda de 200 del stock
        }

        // Verifica si hay suficientes monedas de 100 para el cambio
        while (remainingChange >= 100 && coins100.Count() > 0)
        {
            remainingChange -= 100;
            coins100.Pop(); // Elimina una moneda de 100 del stock
        }

        // Verifica si hay suficientes monedas de 50 para el cambio
        while (remainingChange >= 50 && coins50.Count() > 0)
        {
            remainingChange -= 50;
            coins50.Pop(); // Elimina una moneda de 50 del stock
        }

        // Si remainingChange llega a cero, entonces hay suficiente cambio
        if (remainingChange == 0)
        {
            return true;
        }

        // Si no hay suficientes monedas para dar cambio, retorna false
        return false;
    }
